...
AppPage {
    ...
    property int gridWidth: 400 // width and height of the game grid
    ...
    function createNewTile() {
        ...
        console.log(tileId,randomCellId)
    ...
    function isGameOver() {
        ...
        for (var i = 0; i < gridSizeGame; i++) {
            for (var j = 0; j < gridSizeGame; j++) {
                var index = i * gridSizeGame + j;

                // 检查右边的数字
                if (j < gridSizeGame - 1 && tileItems[index] && tileItems[index + 1] &&
                    tileItems[index].tileValue === tileItems[index + 1].tileValue) {
                    return false; // 可以合并，游戏未结束
                }

                // 检查下边的数字
                if (i < gridSizeGame - 1 && tileItems[index] && tileItems[index + gridSizeGame] &&
                    tileItems[index].tileValue === tileItems[index + gridSizeGame].tileValue) {
                    return false; // 可以合并，游戏未结束
                }
            }
        }

        // 没有空单元格且没有可以合并的数字，游戏结束
        return true;
    }
